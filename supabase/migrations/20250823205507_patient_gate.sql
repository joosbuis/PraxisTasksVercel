/*
  # Initial Database Setup for Praxis Tasks
  ...
  2. Security
    ...
    - Manager-only policies for user management

  3. Default Data
    - Create default manager account (1001)
    - Create default user account (1002)
...
-- Insert default users (these will be created without auth initially)
DO $$
BEGIN
  -- Insert default manager if not exists
  IF NOT EXISTS (SELECT 1 FROM users WHERE employee_number = '1001') THEN
    INSERT INTO users (
      employee_number,
      username,
      name,
  ...
  IF NOT EXISTS (SELECT 1 FROM users WHERE employee_number = '1002') THEN
    INSERT INTO users (
      employee_number,
      username,
      name,
