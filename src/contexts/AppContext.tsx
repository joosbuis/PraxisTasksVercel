...
function setUserTheme(userId: string | undefined, theme: AppSettings["theme"]) {
  try {
    if (userId) localStorage.setItem(`praxis:userTheme:${userId}`, theme);
  } catch {}
}
...
const normalizeTask = (x: any): Task => ({
  ...
  status: ((): TaskStatus => {
    ...
    if (v === "needs-pickup" || v === "needs_pickup") return "needs-pickup";
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // taalstrings voor de huidige taal
  const t = useTranslation(settings.language);

  // ---- Thema & Taal updates ----
  useEffect(() => {
    const theme = currentUser
      ? getUserTheme(currentUser.id) || settings.theme || "light"
  ...
  const setPassword = async (userId: string, newPassword: string) => {
    try {
      ...
      const { data: userData } = await supabase
        .from("users")
        .select("employee_number, username")
        .eq("id", userId)
        .maybeSingle();
      if (!userData) return false;
  ...
  const consumeOneTimeCode = async (employeeNumber: string, code: string): Promise<User | null> => {
    try {
      const emp = (employeeNumber ?? "").trim().replace(/"/g, "");
      const one = (code ?? "").trim();
      const { data, error } = await supabase.rpc("consume_temp_code", {
  ...
  const updateSettings = async (next: Partial<AppSettings> | AppSettings) => {
    ...
    try {
      for (const [key, value] of Object.entries(serverPayload)) {
        await withAuthRetry(() =>
          supabase
            .from("settings")
            .upsert({ key, value: value as any, updated_at: new Date().toISOString() } as any)
        );
      }
    } catch (error) {
    ...
    if (typeof nextTheme !== "undefined") {
      try {
        setUserTheme(currentUser?.id, nextTheme as AppSettings["theme"]);
      } catch {}
    ...
    try {
      ...
      const { error } = await withAuthRetry(() =>
        supabase.from("tasks").insert({
          ...
          assigned_to: safe.assignedTo || null,
          assigned_to_name: safe.assignedToName,
          board: safe.board,
          deadline: safe.deadline || null,
          activities: safe.activities,
          started_by: safe.startedBy || null,
          started_by_name: safe.startedByName || null,
          started_at: safe.startedAt || null,
          picked_up_by: safe.pickedUpBy || null,
          picked_up_by_name: safe.pickedUpByName || null,
          picked_up_at: safe.pickedUpAt || null,
          completed_by: safe.completedBy || null,
          completed_by_name: safe.completedByName || null,
          completed_at: safe.completedAt || null,
        } as any)
      );
      if (error) throw error;
      await fetchTasks();
