      ...
      <div className="max-w-6xl mx-auto">
          ...
          <div className="card p-6 mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-left">
                ...
                <tbody>
                  {filteredUsers.map((u) => {
                    const normalizedRole: AnyRole = String(u.role) === "employee" ? "user" : (u.role as AnyRole);
                    ...
                    return (
                      <tr key={u.id} className="border-t">
                        ...
                        <td className="py-2 px-3">
                          <div className="flex items-center gap-2">
                            <button className="btn-secondary" type="button" onClick={() => setShowUserEdit(u.id)}>
                            ...
                            <button
                              ...
                              type="button"
                              onClick={() => {
                                if (u.id === currentUser.id) { alert("Je kunt jezelf niet verwijderen."); return; }
                                if (confirm(`Verwijder ${u.name ?? u.employeeNumber}?`)) handleUserDelete(u.id);
                              }}
                            >
          ...
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
              <div className="flex items-center justify-between p-6 border-b border-gray-200">
                ...
                <button onClick={() => setShowCreateUser(false)} className="text-gray-400 hover:text-gray-600" type="button">
                  <X className="w-6 h-6" />
                </button>
              </div>
              <div className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">{tt.employeeNumber} *</label>
                  <input className="input-field" placeholder="Bijv. 3001" value={newUser.employeeNumber}
                    onChange={(e) => setNewUser(p => ({ ...p, employeeNumber: e.target.value }))} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">{tt.name} *</label>
                  <input className="input-field" placeholder="Voor- en achternaam" value={newUser.name}
                    onChange={(e) => setNewUser(p => ({ ...p, name: e.target.value }))} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">{tt.role}</label>
                  <select className="input-field" value={newUser.role}
                    onChange={(e) => setNewUser(p => ({ ...p, role: (e.target.value as AnyRole) || "user" }))}>
                    <option value="user">{tt.employee}</option>
                    <option value="manager">{tt.manager}</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">{tt.departments}</label>
                  <div className="space-y-2">
                    <label className="flex items-center">
                      <input type="checkbox" className="rounded mr-2"
                        checked={newUser.boards.includes("voorwinkel")}
                        onChange={() => setNewUser(p => ({ ...p, boards: toggleBoardIn(p.boards, "voorwinkel") }))} />
                      {tt.voorwinkel}
                    </label>
                    <label className="flex items-center">
                      <input type="checkbox" className="rounded mr-2"
                        checked={newUser.boards.includes("achterwinkel")}
                        onChange={() => setNewUser(p => ({ ...p, boards: toggleBoardIn(p.boards, "achterwinkel") }))} />
                      {tt.achterwinkel}
                    </label>
                  </div>
                </div>
              ...
              <div className="flex justify-end gap-3 p-6 border-t border-gray-200">
                <button onClick={() => setShowCreateUser(false)} className="btn-secondary" type="button">{tt.cancel}</button>
                <button
                  className="btn-primary" type="button"
                  onClick={async () => {
                    if (!newUser.employeeNumber || !newUser.name) { alert("Vul alle verplichte velden in"); return; }
                    if (!newUser.boards.length) { alert("Selecteer minimaal één afdeling"); return; }
                    if (users.find((u) => u.employeeNumber === newUser.employeeNumber)) { alert("Dit personeelsnummer bestaat al"); return; }
                    const ok = await createUser({
                      employeeNumber: newUser.employeeNumber.trim(),
                      name: newUser.name.trim(),
                      role: (String(newUser.role) === "manager" ? "manager" : "user") as any,
                      boards: newUser.boards,
                    });
                    if (ok) { setShowCreateUser(false); setNewUser({ employeeNumber: "", name: "", role: "user", boards: ["voorwinkel"] }); }
                    else { alert("Aanmaken mislukt (bestaat al of serverfout)."); }
                  }}
                >
                  {tt.create} {tt.users}
                </button>
        ...
        {isManager && showUserEdit && (
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
              <div className="flex items-center justify-between p-6 border-b border-gray-200">
                <h2 className="text-xl font-semibold text-gray-900">{tt.edit} {tt.users}</h2>
                <button onClick={() => setShowUserEdit(null)} className="text-gray-400 hover:text-gray-600" type="button">
                  <X className="w-6 h-6" />
                </button>
              </div>
              <div className="p-6">
                {(() => {
                  ...
                  if (!user) return null;
                  const normalizedRole: AnyRole = String(user.role) === "employee" ? "user" : (user.role as AnyRole);
                  return (
                    <div className="space-y-4">
                      ...
                      {(user as any).temporaryCode && (
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">{tt.temporaryCode}</label>
                          <div className="bg-gray-100 p-3 rounded-lg flex items-center gap-2">
                            <Key className="w-4 h-4 text-orange-600" />
                            <code className="font-mono text-lg">{(user as any).temporaryCode}</code>
                            <button className="ml-auto btn-secondary text-xs py-1 px-2" type="button"
              ...
              <div className="flex justify-end gap-3 p-6 border-t border-gray-200">
                <button onClick={() => setShowUserEdit(null)} className="btn-secondary" type="button">{tt.close}</button>
              </div>
            </div>
          </div>
        )}
